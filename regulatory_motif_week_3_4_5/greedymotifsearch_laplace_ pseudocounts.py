...
def build_profile_laplace(motifs, k):
    """Pseudocounts allow to find more correct probabilities: a zero in one pos will not kill that probability"""
    t = len(motifs)
    profile = {"A": [], "C": [], "G": [], "T": []}
    for pos in zip(*motifs):
        ...
        for nuc in "ACGT":
            ...
            profile[nuc].append((counts.get(nuc, 0) + 1) / (t + 4)) #.get(nuc, 0) means "give me the count of this nucleotide, or 0 if it doesn't exist in the Counter." Without it you'd get a KeyError every time a nucleotide doesn't appear
    return profile
...
def greedy_motif_search(dna_list, k, t):
    """Using pseudocounts is more accurate than greedy search alone"""
    ...
    for i in range(len(dna_list[0]) - k + 1):
        ...
        for j in range(1, t): #For each subsequent sequence, builds profile from current motifs and finds most probable k-mer
            profile = build_profile_laplace(motifs, k)
...
k, t, dna_list = read_file("regulatory_motif_week_3_4_5/greedy_motif.txt")
