...
and origin of replication that is implied by the 9-mer motifs at most 500 bp apart"""
...
for i in range(len(positions)):
        for j in range(len(positions)):
            if i != j:
                if abs(positions[j]-positions[i])<500: #absolute value difference
                    store.add(positions[i])
                    store.add(positions[j])

print("9-mer positions at most 500 bp apart:", *store)

#form clumps, i.e., appear close to each other in a small region of the genome. You may check that the same conclusion is reached when searching for ATGATCAAG (its reverse complement, another 9-mer that DNA A binds in the opposite strand)
...
#note: different bacteria may use different DnaA boxes (9-mers)
